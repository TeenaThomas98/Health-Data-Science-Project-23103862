...
for (state in state_names) {
  ...
  saveWidget(map, paste0(state, "Statelevel_SGA_GROW_map.html"), selfcontained = TRUE)
}
...
create_state_map <- function(state_name, cleaned_data_merged) {
  ...
  # Create the map
  ggplot() +
    geom_sf(data = state_data, aes(fill = `prevalence of SGA`), color = NA, size = 0.1) +
    scale_fill_viridis(option = "plasma", name = "SGA Prevalence (%)", direction = -1,
                       na.value = "grey50",
                       guide = guide_colorbar(barwidth = 15, barheight = 0.5), limits = c(0, 60)) +
...
create_state_map <- function(state_name, cleaned_data_merged) {
  ...
  # Create the map
  ggplot() +
    geom_sf(data = state_data, aes(fill = prevalence_of_SGA), color = NA, size = 0.1) +
    scale_fill_viridis(option = "plasma", name = "SGA Prevalence (%)", direction = -1,
                       na.value = "grey50",
                       guide = guide_colorbar(barwidth = 15, barheight = 0.5), limits = c(0, 70)) +
    labs(title = paste("Prevalence of Small for Gestational Age (SGA) in", state_name),
         subtitle = "Based on INTERGROWTH-21st Standard") +
    theme_minimal() +
    theme(
...
for (state in state_names) {
  map <- create_interactive_state_map(state, cleaned_data_merged)
  saveWidget(map, paste0(state, "INTERGROWTH_SGA_state_level_map.html"), selfcontained = TRUE)
}
...
for (state in state_names) {
  map <- create_interactive_state_map(state, cleaned_data_merged)
  saveWidget(map, paste0(state, "LBW_district_map.html"), selfcontained = TRUE)
}
...
create_state_map <- function(state_name, cleaned_data_merged) {
  ...
  # Create the map
  ggplot() +
    geom_sf(data = state_data, aes(fill = COLUMN2), color = NA, size = 0.1) +
    scale_fill_viridis(option = "plasma", name = "LBW Prevalence (%)", direction = -1, 
                       na.value = "grey50",
                       guide = guide_colorbar(barwidth = 15, barheight = 0.5), limits = c(0, 40)) +
